...
def cache_with_redis(prefix, ttl=QUIZ_CACHE_TTL):
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not user_data:
                return f(*args, **kwargs)  # Skip caching if no user data
            
            # Create a cache key based on the function name, user role, username, and arguments
            cache_key = f"{prefix}:{f.__name__}:{user_data['role']}:{user_data['username']}:"
...
@app.route('/quiz/create-quiz', methods=['POST'])
...
def create_quiz():
    ...
    # Add teacher information to the quiz
    data["teacher_username"] = user_data["username"]
    ...
    try:
        ...
        logger.info("Clearing quiz-related caches after new quiz creation")
        
        # Get all keys in Redis
        all_keys = redis_client.keys("*")
        logger.info(f"Total keys in Redis: {len(all_keys)}")
        
        # Clear all quiz-related caches with more specific patterns
        cache_patterns = [
            f"quiz_listing:get_quizzes:teacher:{user_data['username']}:*",  # Teacher's own quizzes
            f"quiz_listing:get_quizzes:student:*",  # All student caches (since they might be mapped to this teacher)
            "quiz_detail:*",              # Quiz details
            "user_results:*",             # User results
            "quiz:*"                      # Any quiz-related keys
        ]
        ...
        for pattern in cache_patterns:
            ...
            if keys:
                ...
                for key in keys:
                    ...
                    logger.info(f"Clearing key: {key_str}")
                    redis_client.delete(key)
                    cleared_keys.append(key_str)
            else:
                logger.info(f"No keys found for pattern '{pattern}'")
        
        # Log summary of cleared keys
        logger.info(f"Cleared {len(cleared_keys)} keys in total")
        if cleared_keys:
            logger.info("Cleared keys: " + ", ".join(cleared_keys))
...
def get_quizzes():
    ...
    # Build query based on user role and relationships
    if user_data["role"] == "student":
        ...
        logger.info(f"Student {user_data['username']} is mapped to teachers: {teacher_usernames}")
        
        # Add teacher filter to query
        query["teacher_username"] = {"$in": teacher_usernames}
    elif user_data["role"] == "teacher":
        # Teachers can only see their own quizzes
        query["teacher_username"] = user_data["username"]
        logger.info(f"Teacher {user_data['username']} viewing their own quizzes")
    
    # Add subject and level filters if specified
    if subject:
    ...
    try:
        # Get total count of all quizzes (for debugging)
        total_quizzes = quiz_collection.count_documents({})
        logger.info(f"Total quizzes in database: {total_quizzes}")
        
        # Get filtered quizzes using MongoDB query
        quizzes = list(quiz_collection.find(query))
        logger.info(f"Found {len(quizzes)} quizzes matching query")
...
def get_quiz(quiz_id):
    ...
    try:
        ...
        if user_data["role"] == "student":
            # Check if student is mapped to the teacher who created this quiz
            mapping = teacher_student_mapping.find_one({
                "teacher_username": quiz["teacher_username"],
                "student_username": user_data["username"]
            })
            if not mapping:
                return jsonify({"error": "You don't have access to this quiz"}), 403
        elif user_data["role"] == "teacher" and quiz["teacher_username"] != user_data["username"]:
            return jsonify({"error": "You don't have access to this quiz"}), 403
...
def update_quiz(quiz_id):
    ...
    try:
        ...
        if quiz["teacher_username"] != user_data["username"]:
            return jsonify({"error": "You don't have permission to update this quiz"}), 403
        ...
        if result.modified_count == 0:
            return jsonify({"error": "No changes made to the quiz"}), 400
        
        updated_quiz = quiz_collection.find_one({"_id": ObjectId(quiz_id)})
        updated_quiz["_id"] = str(updated_quiz["_id"])
        # Clear quiz listing and detail cache after update
        listing_keys = redis_client.keys("quiz_listing:*")
        detail_keys = redis_client.keys("quiz_detail:*")
        if listing_keys:
            redis_client.delete(*listing_keys)
...
def delete_quiz(quiz_id):
    ...
    try:
        ...
        if quiz["teacher_username"] != user_data["username"]:
            return jsonify({"error": "You don't have permission to delete this quiz"}), 403
        ...
        # Clear quiz listing and detail cache after delete
        listing_keys = redis_client.keys("quiz_listing:*")
        detail_keys = redis_client.keys("quiz_detail:*")
        if listing_keys:
            redis_client.delete(*listing_keys)
...
# cache invalidation
@app.route('/invalidate-student-cache/<username>', methods=['POST'])
def invalidate_student_cache(username):
    try:
        logger.info(f"Invalidating cache for student: {username}")
        
        # Clear all quiz-related caches for this student
        cache_patterns = [
            f"quiz_listing:get_quizzes:student:{username}:*",  # Student's quiz listings
            f"quiz_detail:*",                                  # Quiz details
            f"user_results:{username}:*",                      # Student's results
            f"quiz:*"                                          # Any quiz-related keys
        ]
        ...
        return jsonify({
            "success": True,
            "message": f"Cleared {len(cleared_keys)} cached items for student {username}"
        })
    except Exception as e:
        logger.error(f"Error clearing student cache: {str(e)}")
        return jsonify({"error": str(e)}), 500
