...
import TeacherStudentMapping from '../components/TeacherStudentMapping';
...
const Dashboard = () => {
  ...
  const renderActiveSection = () => {
    switch (activeSection) {
      ...
      case 'mapping':
        return userData && <TeacherStudentMapping userRole={userData.role} userData={userData} />;
      case 'content':
      default:
        return (
    ...
    <div className="dashboard-wrapper">
      ...
      <div className="dashboard-nav animate-slide-up">
        ...
        {(userData.role === 'teacher' || userData.role === 'student') && (
          <button
            className={`nav-button ${activeSection === 'mapping' ? 'active' : ''}`}
            onClick={() => setActiveSection('mapping')}
          >
            <span className="nav-icon">👥</span>
            <span className="nav-text">
              {userData.role === 'teacher' ? 'My Students' : 'My Teachers'}
            </span>
          </button>
        )}
      </div>
